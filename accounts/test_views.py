...
class TestAccountsLoginView(TestCase):
    ...
    def test_login_with_incorrect_password(self):
        new_user = User.objects.create_user('testuser', 'testuser@domain.com', 'password')
        response=self.client.post('/accounts/login/', data={'username': 'testuser', 'password': 'none'}, follow=True)
        self.assertTemplateUsed(response, "login.html")
        self.assertIn(b'Your username and password is incorrect', response.content)
...
class TestAccountsLogoutView(TestCase):
    ...
    def test_logout_state(self):
        new_user = User.objects.create_user('testuser', 'testuser@domain.com', 'password')
        self.client.login(username='testuser', password='password')
        page = self.client.get("/accounts/logout/", content_type="html/text", follow=True)
