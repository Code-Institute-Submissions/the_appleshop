...
class TestAccountsForms(TestCase):
    ...
    def test_missing_passwords_registration_form(self):
        response=self.client.post('/accounts/register/', data={'username': 'testuser', 'email': 'testuser@domain.com', 'password1': 'password', 'password2': ''})
        self.assertIn(b'This field is required.', response.content)


    def test_not_matching_passwords_registration_form(self):
        response=self.client.post('/accounts/register/', data={'username': 'testuser', 'email': 'testuser@domain.com', 'password1': 'password', 'password2':'none'})
